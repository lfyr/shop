...
<div class="container wx_wrap mini-innner">
    ...
    <div class="row">
        <div class="col-md-12">
        <form id='address' action="<?php echo U('address/edit');?>" method="post">
        <input type="hidden" name="id" value="<?php echo $data['id']; ?>" />

            <div class="address_new">
                <p><label for="name"><span class="tit">收货人</span><input type="text" value="<?php echo ($data["shr_name"]); ?>" name="shr_name" id="receiver" placeholder="名字"></label></p>
                <p><label for="mobile"><span class="tit">手机号码</span><input type="text" value="<?php echo ($data["shr_tel"]); ?>" name="shr_tel" id="mobile" placeholder="手机号码"></label></p>
                <p>
                    <label for="provinceId">
                        ...
                        <select class="combox sele"  id="province" name="shr_province">
                            ...
                            <?php foreach($province as $k=>$v): if($data['shr_province']==$v['province']){ $select='selected="selected"'; }else{ $select=''; } ?>
                                <option <?php echo ($select); ?> provinceid="<?php echo ($v["provinceid"]); ?>" value="<?php echo ($v["province"]); ?>"><?php echo ($v["province"]); ?></option>
                            <?php endforeach;?>
                        </select>
                    </label>
                </p>
                ...
                <p><label for="adinfo"><span class="tit">详细地址</span><input name="shr_address" id="subAdds" value="<?php echo ($data["shr_address"]); ?>" type="text" placeholder="详细地址"></label></p>
            </div>
        </form>
...
var contextPath = '';
    $("#province").change(function(){
        ...
        var city1='<?php echo $data['shr_city']?>';
        $.ajax({
            type:"GET",
            ...
            success:function(data){
                ...
                $(data).each(function(k,v){
                    ...
                    if(city1 == v.city){
                      html+="<option selected='selected' cityid="+v.cityid+" value="+v.city+">"+v.city+"</option>";
                    }else{
                ...
                $('#city').html(html);
                $("select[name=shr_city]").trigger("change");
            }
        });
    });
    ...
    $("#city").change(function(){
        ...
        var area1='<?php echo $data['shr_area']?>';
        $.ajax({
            type:"GET",
            ...
            success:function(data){
                ...
                $(data).each(function(k,v){
                    if(area1 == v.area){
                      html+="<option selected='selected' cityid="+v.areaid+" value="+v.area+">"+v.area+"</option>";
                    }else{
...
$("select[name=shr_province]").trigger("change");
</script>
</body>
</html>